...
local function recent_sessions(start, cwd)
    ...
    for i, session in ipairs(sessions) do
        ...
        if end_index then
            local short_fn
            if cwd then
                short_fn = vim.fn.fnamemodify(session.fn, ":.")
            else
                short_fn = vim.fn.fnamemodify(session.fn, ":~")
            end
...
local function config_dashboard()
    local banner = {
        [[                                                                   ]],
        [[      ████ ██████           █████      ██                    ]],
        [[     ███████████             █████                            ]],
...
return {
    ...
    config = function()
        theme.alpha_banner()
        require("alpha").setup(config_dashboard())
    end,
    keys = {
        { "<leader>;", "<cmd>Alpha<cr>", desc = theme.icons.dashboard .. "Dashboard" },
    },
}
